          ...
          {CTV_FORMATS.map((fmt, i) => {
            ...
            return (
              <motion.div key={fmt.name} custom={i} variants={cardVariant} initial="hidden"
                ...
                className="glass-dark rounded-2xl p-6 border border-brand-purple/[0.10] group relative overflow-hidden"
                style={{ boxShadow: "0 8px 40px rgba(7,17,47,0.08), 0 0 0 1px rgba(139,92,246,0.08)" }}>
                ...
                <div className="relative z-10">
                  <div className="flex items-start justify-between mb-4">
                    ...
                    {fmt.badge && (
                      <span className="text-[10px] font-bold px-2.5 py-1 rounded-full bg-brand-purple/20 text-brand-purple border border-brand-purple/30">
                        {fmt.badge}
                      </span>
                    )}
                  </div>

                  <h3 className="text-[15px] font-bold text-text-primary mb-2 leading-snug">{fmt.name}</h3>
                  <p className="text-[12px] text-text-secondary leading-relaxed mb-4">{fmt.description}</p>

                  <div className="flex items-center gap-3">
                    <div className="flex-1 glass-dark rounded-xl p-2.5 border border-brand-purple/[0.10]">
                      <p className="text-[9px] text-text-muted uppercase tracking-wider mb-0.5">CPM Range</p>
                      <p className="text-[13px] font-bold text-brand-purple">{fmt.cpm}</p>
                    </div>
                    <div className="flex-1 glass-dark rounded-xl p-2.5 border border-brand-purple/[0.10]">
                      <p className="text-[9px] text-text-muted uppercase tracking-wider mb-0.5">Completion</p>
